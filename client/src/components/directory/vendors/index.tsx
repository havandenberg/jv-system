...
const gridTemplateColumns = '30px 0.5fr 2fr 1fr 1.5fr 30px';

const VendorDirectory = ({ actions }: SubDirectoryProps) => {
  const { Search } = useSearch();
  const { data, loading, error } = api.useVendors();
  const items = data ? data.nodes : [];

  const columnLabels = useColumns<Vendor>(
    'vendorName',
    SORT_ORDER.ASC,
    listLabels,
    'directory',
  ...
  const [
    allSelectedItems,
    { selectVendor, isAllVendorsSelected, toggleAllVendors },
  ] = useDirectorySelectionContext();

  const selectedItems = allSelectedItems.vendors;

  return (
    <Page
      actions={actions}
      breadcrumbs={breadcrumbs('vendors')}
      extraPaddingTop={117}
      headerChildren={
        <>
              ...
              item && (
                <div key={key} style={style}>
                  <ListItem<Vendor>
                    ...
                    onSelectItem={() => selectVendor(item)}
                    selected={!!selectedItems.find((it) => it.id === item.id)}
                    to={`/directory/vendors/${item.id}`}
                  />
                </div>
              )
            );
