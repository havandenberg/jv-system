...
import {
  filterCustomerProgramEntries,
  getShipperAllocatedPalletCount,
  getAvailablePalletCount,
  sortAllocatedCustomerProgramEntries,
...
const CustomerProgramEntryRow = ({
  ...
  const currentAllocatedCount = `${
    getShipperAllocatedPalletCount(entry, shipperProgramEntry.id) || 0
  }`;
...
const getInitialAllocateState = (
  entry: Maybe<ShipperProgramEntry> | undefined,
  customerProgramEntries: CustomerProgramEntry[],
) =>
  entry
    ? customerProgramEntries
        ...
        .reduce((acc, curr) => {
          ...
          ).find((a) => a && a.shipperProgramEntryId === entry.id);
...
const ShipperProgramAllocateModal = ({
  disabled,
  customerProgramEntries,
  entry,
  loading,
  ...
  const previousCustomerProgramEntries = usePrevious(customerProgramEntries);

  const [focused, setFocused] = useState(false);
  const [search, setSearch] = useSearchQueryParam('customerAllocateSearch');
  ...
  const [allocateState, setAllocateState] = useState<{
    [key: number]: { entry: CustomerProgramEntry; allocatedCount: string };
  ...
  const nonZeroStateValues = values(allocateState).filter(
    (val) =>
      existingAllocations.find(
        (alloc) => alloc.customerProgramEntryId === val.entry.id,
      ) || parseInt(val.allocatedCount, 10) > 0,
  );

  const [{ customerAllocateSearch }] = useProgramsQueryParams();

  const allEntries = [
    ...sortAllocatedCustomerProgramEntries(nonZeroStateValues),
    ...sortCustomerProgramEntries(
      filterCustomerProgramEntries(
        customerProgramEntries,
        nonZeroStateValues,
      ...
      return {
        addedAllocations: !!existingAllocation
          ...
          : [
              ...acc.addedAllocations,
              {
                shipperProgramEntryId: entry?.id,
                customerProgramEntryId: val.entry.id,
  ...
  useEffect(() => {
    if (
      !equals(previousCustomerProgramEntries, customerProgramEntries) ||
    ...
    loading,
    previousLoading,
    previousCustomerProgramEntries,
    customerProgramEntries,
    initialAllocateState,
  ]);
              ...
              <l.Flex alignCenter justifyBetween>
                {isDirty ? (
                  <ResetButton
                    ...
                    onClick={() => {
                      setAllocateState(
                        getInitialAllocateState(entry, customerProgramEntries),
                      );
                    }}
                  >
            ...
            {allEntries.length > 0 ? (
              <VirtualizedList
                ...
                rowRenderer={({ index, style }) => {
                  const item: CustomerProgramEntry = allEntries[index];
                  const program = item?.customerProgram as CustomerProgram;
                  return (
                    item && (
                      <RowWrapper
                        allocated={
                        ...
                        style={style}
                      >
                        <CustomerProgramEntryRow
                          allocateCount={
                            allocateState[item.id]?.allocatedCount || '0'
                          }
                          shipperProgramEntry={entry as ShipperProgramEntry}
                          entry={item}
...
export default ShipperProgramAllocateModal;
