const { db2UpdateTable } = require('.');

// db2UpdateTable('directory/country');
// db2UpdateTable('directory/shipper');
// db2UpdateTable('directory/customer');
// db2UpdateTable('directory/warehouse');
// db2UpdateTable('directory/vendor');

// db2UpdateTable('product/vessel');
// db2UpdateTable('product/inventory-item');
db2UpdateTable('product/pallet');
// db2UpdateTable('product/pallet-section');

// db2UpdateTable('operations/order/master');
// db2UpdateTable('operations/order/item');
// db2UpdateTable('operations/truck-load');

// db2UpdateTable('accounting/invoice/header');
// db2UpdateTable('accounting/invoice/item');

// db2UpdateTable('accounting/expense/header');
// db2UpdateTable('accounting/expense/item');

// db2UpdateTable('product/master');
// db2UpdateTable('product/species');
// db2UpdateTable('product/variety');
// db2UpdateTable('product/size');

// db2UpdateTable('product/pack-master');
// db2UpdateTable('product/pack-atmosphere');
// db2UpdateTable('product/pack-box-style');
// db2UpdateTable('product/pack-box-type');
// db2UpdateTable('product/pack-destination');
// db2UpdateTable('product/pack-grade');
// db2UpdateTable('product/pack-hold');
// db2UpdateTable('product/pack-label');
// db2UpdateTable('product/pack-liner');
// db2UpdateTable('product/pack-out');
// db2UpdateTable('product/pack-pallet-type');
// db2UpdateTable('product/pack-production');
// db2UpdateTable('product/pack-special');
// db2UpdateTable('product/pack-style');
// db2UpdateTable('product/pack-tree-ripe');
